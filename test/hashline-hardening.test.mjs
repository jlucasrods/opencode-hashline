...
test("system instruction is config-aware and batch-first", () => {
  ...
  assert.match(instruction, /For existing files, prefer read \+ apply_patch_hashline/i)
...
test("tool descriptions guide agents toward batched edit workflows", async () => {
  ...
  assert.match(readOutput.description, /prefer apply_patch_hashline for existing-file changes/i)
  ...
  assert.match(editOutput.description, /Prefer one batched apply_patch_hashline call per existing file/i)
  ...
  assert.match(writeOutput.description, /Prefer apply_patch_hashline for targeted existing-file changes/i)
})
